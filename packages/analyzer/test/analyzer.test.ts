...
tRef({ empty: '暂无数据', nested: [null, '重试'] })`,
  ...
  it('extracts every supported tComputed input', () => {
    const module = analyzeModule(
      `import { tComputed as optionsText } from 'virtual:ai-i18n'
const name = getName()
optionsText('保存', { comment: '按钮' })
optionsText\`你好 \${name}\`
optionsText({ buttons: { cancel: '取消' }, states: ['等待中'] })`,
      'OptionsPanel.ts',
    );

    expect(extractMessages(module)).toMatchObject({
      messages: [
        { source: '保存', comment: '按钮' },
        { source: '你好 {{0}}' },
        { source: '取消' },
        { source: '等待中' },
      ],
