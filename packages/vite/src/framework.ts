...
function frameworkTypes(
  ...
  const adapter = adapterFramework
    ? `  export const useI18n: ReturnType<typeof import('@ai-i18n/vite/${adapterFramework}').create${capitalize(adapterFramework)}I18n>;\n`
    : '';
  const globals = autoImport
    ? AUTO_IMPORTS[framework]
        .map((name) => {
          const type =
            name === 'useI18n'
              ? `ReturnType<typeof import('@ai-i18n/vite/${adapterFramework}').create${capitalize(adapterFramework!)}I18n>`
              : `import('@ai-i18n/vite').I18nRuntime['${name}']`;
          return `declare const ${name}: ${type};`;
        })
        .join('\n')
    : '';

  return `// Generated by @ai-i18n/vite. Do not edit.
