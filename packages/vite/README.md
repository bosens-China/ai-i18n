...
Agent 补译可通过 `use-ai-i18n-mcp` Skill 从应用入口扫描静态依赖、懒加载与 glob，
无需完整 Build，也不请求 Provider。扫描使用 Vite Dev 转换，保留历史译文；
Build 专用的页面生成和生产集成仍需真实 Build 验证。没有 Host MCP 连接时，Skill 可以通过
目标应用安装的 `@ai-i18n/mcp` 执行普通补译，不必注册服务器。

成功 Build 默认汇总去重文案数、含文案的文件数，以及每个目标语言的已翻译/未翻译数量。
统计等待 Provider 和持久化完成，包含人工覆盖；同一消息所有使用位置均有译文才算完成。
...
补译 Skill 和 Agent 插件使用同一入口扫描：已有 Dev 时由该进程处理，否则建立临时 Vite 转换实例。
全部页面校对和独立 Review 页按需扫描可达的未访问页面；当前页面模式不因打开工作台启动全量扫描。
源码改动会自动刷新列表和位置；解析失败显示过期状态，恢复后重试。扫描缓存位于 `cacheDir`，
只复用提取结果，译文/人工覆盖每次重读。自定义转换插件等无法证明输入稳定时保守重扫。
Agent 插件打包与安装边界见 [plugins/ai-i18n](../../plugins/ai-i18n/README.md)。
