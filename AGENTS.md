...
- `apps/docs` 面向应用开发者，应优先说明如何接入、使用和排障，只在理解行为所必需时简要说明内部流程，不暴露 Agent 专用字段、协议细节或实现细节；`.agents/skills` 面向 AI Agent，应保留完成任务所需的精确工具契约、操作边界与验证步骤。编写或同步时必须区分两类受众，避免互相复制。
