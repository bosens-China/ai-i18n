...
- 首个非 prerelease 稳定版本发布前，必须完成 [`docs/plans/remove-legacy-translation-memory/PRD.md`](./docs/plans/remove-legacy-translation-memory/PRD.md)：删除代码中标记为 `TODO(stable-release)` 的 `translations.json` 单文件兼容逻辑、JSON 模式旧 `storage.json` 标记兼容，以及相关旧路径、测试和文档，不得将这些兼容层带入正式稳定版
