...
用户文档站（Rspress）。内部 PRD 与 TODO 仍在仓库根目录 `docs/`。

```sh
# 在仓库根目录 — 一键：build 插件包 + 三个 example dev + rspress dev
pnpm docs:dev
...
- **开发**：demo 页 iframe 指向 `http://localhost:5188x/`（见 `apps/docs/components/demo-urls.ts`）
