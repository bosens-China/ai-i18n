...
```sh
pnpm add @boses/openai
```

`@boses/openai` 连接 OpenAI-compatible API。它不会替你选择供应商或模型。
...
完整类型与约束见 [`@boses/openai` API](../api/openai/)。
...
也可以实现 `@boses/core` 的 `Translator`，自行对接其他翻译后端。返回值必须覆盖每个
