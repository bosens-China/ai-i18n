...
- Agent Skills 告诉 Agent 如何接入 `@boses/vite`，以及如何按安全流程补译。
...
[mcp_servers.ai_i18n]
command = "npx"
args = ["-y", "@boses/mcp", "--root", "/absolute/path/to/repo"]
```
...
```sh
claude mcp add --transport stdio --scope local ai-i18n -- npx -y @boses/mcp --root /absolute/path/to/repo
```
