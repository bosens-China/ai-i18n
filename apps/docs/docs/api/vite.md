...
Vite 与 ESLint 共用 `AI_I18N_DIAGNOSTIC_LOCALE`：

| 值      | 行为                                                          |
| ------- | ------------------------------------------------------------- |
| `auto`  | 根据 Node 时区选择；`Asia/Shanghai`、`Asia/Urumqi` 使用中文。 |
| `zh-CN` | 固定使用简体中文。                                            |
| `en-US` | 固定使用英文。                                                |

未设置时等同于 `auto`，其他时区默认使用英文。该变量只影响开发服务器和构建期间的提示与
报错，不影响浏览器 Runtime、locale 文件或翻译结果。
...
| 字段          | 类型     | 必填 | 默认值 | 作用                                                           |
| ------------- | -------- | ---- | ------ | -------------------------------------------------------------- |
| `maxMessages` | `number` | 否   | 不限制 | `translations.json` 中 `messages` 允许保留的最大条数。         |
