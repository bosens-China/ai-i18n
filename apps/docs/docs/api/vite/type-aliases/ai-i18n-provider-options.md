...
| `logging`        | `boolean \| string`                               | 否   | `false`   | 关闭日志，或启用并选择日志目录。              |
...
日志默认关闭。`logging: true` 使用 Vite root 下的 `logs/`；字符串指定目录，相对路径基于 Vite
root，绝对路径保持不变。空字符串无效。开启后，Vite 会把解析后的目录传给 Translator 和批次
...
Provider 的完整接入流程见 [AI 翻译](/ai/ai-translation)。
