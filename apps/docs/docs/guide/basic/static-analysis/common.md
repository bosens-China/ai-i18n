...
不要把 Runtime `t` 作为函数参数或局部函数值再次传递。即使参数标注为 `I18nRuntime['t']`，
静态分析也不会根据 TypeScript 类型推断它的运行时来源，并会给出诊断。普通工具模块应直接从
`virtual:ai-i18n` 导入 `t`；组件代码也可以直接使用 `useI18n()` 返回的 `t`。
