...
由后续正常存储读取或写入完成。如果旧版开发服务器报
`ENOENT ... .transaction.json`，这是文件变更通知到达后文件已消失的热更新读取竞态，
仅凭这一错误不能判断译文损坏。真实分片在事件排队后消失时，插件仍会同步最新译文状态。
旧版遇到此问题时，可尝试重启开发服务临时恢复，并升级到包含该修复的插件版本。
不要通过补建事务文件或忽略整个
译文目录来处理，否则可能影响事务恢复或后续译文更新。

`logs/` 与 `*.log` 是 OpenAI Provider 的本地日志，可能包含业务文案。使用自定义日志目录时也要
忽略该目录。日志内容与分享前检查见[LLM 日志与排障](/guide/advanced/llm-logs)。
...
[翻译 Skill](/guide/advanced/ai-tools) 从应用入口扫描，无需先运行完整 Build。
...
扫描与完整 Build 都会按所选入口重建清单并迁移旧的 extracted 文件命名；扫描保留历史译文，
不执行孤立和容量清理。

Vite Dev 的翻译校对页面在当前页面模式使用 Dev 结果；全部页面和独立页会按需刷新入口可达的
完整清单，无需先 Build。修改未访问页面也会更新已打开的完整校对列表。扫描缓存位于 Vite
`cacheDir`，只用于跳过重复提取，可以删除重建；译文和人工覆盖始终读取项目中的最新文件。

成功 Build 默认输出文案总数与各语言的已翻译、未翻译数量，包含人工覆盖。
可用 `diagnostics.buildSummary: false` 关闭；启用性能诊断时会合并输出，见
...
- **值不同**：停止当前操作，保留冲突文件，并提示文件路径和两处行列位置。请确认最终值后
  只保留一个键，再重试；不会默认采用最后一个值，也不会通过 Build 强行覆盖。

这套检查默认生效，无需配置。Dev、入口扫描和 Build 在正常同步时处理各自写入的文件；
校对保存、MCP 写入和 Provider 保存译文共用存储层，不必为了清理额外执行完整 Build。
