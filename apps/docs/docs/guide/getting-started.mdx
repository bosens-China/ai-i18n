...
<PackageManagerTabs command="install -D unplugin-auto-import" />
...
[ESLint 9 Flat Config](/guide/eslint)。

## 下一步

1. 阅读[框架上手](/guide/frameworks)，选择 Vue、React 或 Vanilla 的组件写法。
2. 阅读[文件与工作流](/guide/workflow)，了解 Git 提交约定。
3. 需要模型自动补译时，继续阅读 [AI 翻译](/guide/ai-translation)。
4. 所有字段和函数签名见[配置与 API](/api/vite)。
