...
完成 Skill 与 MCP 接入后，可以把下面的提示词发给 Agent：
...
处理单个语言或文件时，可改用更窄的提示词：

<Prompt
  eyebrow="给 Agent"
  title="按文件 / 语言补译"
  description="适合只补某一文件或某一目标语言。"
  prompt={`使用 use-ai-i18n-mcp，只补 apps/web/src/pages/Home.vue 的日语翻译。
...
所有工具的必填参数、可选参数、默认值与返回结构见 [MCP 工具 API](/guide/mcp)。
