...
| `no-embedded-markup`            | warning  | 通用           | 避免让静态 HTML/SVG 结构进入翻译源文。         |
...
如果局部参数或函数值标注为 `I18nRuntime['t']`，规则会提示它不属于可提取的 Runtime binding。
请直接调用从 `virtual:ai-i18n` 导入的 `t`，或直接使用 `useI18n()` 返回的 `t`。
...
[通用文案写法](/guide/basic/static-analysis/common#支持的参数)。规则不限制长文案、多行文案、
...
新项目优先使用与 Vite 框架模式一致的预设。自动导入项目分别使用
`vanilla-auto-import`、`vue-auto-import` 或 `react-auto-import`；预设已包含对应规则，
不需要手动复制其内部配置。框架自动导入 API 见 [自动导入](/guide/basic/auto-import)。
...
需要统一自动导入风格时，再启用 `no-redundant-auto-import`。该规则支持 `eslint --fix`；完整
选项见下文。
...
| 选项                  | 适用规则与行为                                                                                                            |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `autoImport`          | 六条检查规则接受布尔值或各自支持的 API 数组；`no-redundant-auto-import` 只接受非空 API 数组。                             |
| `framework`           | `no-eager-translation` 接受 `vue`；`no-unsubscribed-t` 与 `no-unsubscribed-runtime-state` 接受 `vue` 或 `react`。         |
| `tsconfigPath`        | 仅适用于 `t-static-args`、`no-embedded-markup`、`no-eager-translation`、`no-unsubscribed-t` 和 `static-candidate-limit`。 |
...
`getLangLoadState` 与 `i18nComputed`。框架项目应优先使用预设；手动配置时参考对应规则的
API 类型声明。
