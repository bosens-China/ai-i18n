...
Rspress Build 会在文档站根目录生成
[`llms.txt`](https://bosens-china.github.io/ai-i18n/llms.txt) 和
[`llms-full.txt`](https://bosens-china.github.io/ai-i18n/llms-full.txt)：

- `llms.txt` 是带页面说明的精简索引。建议 Agent 先读它，再打开与当前框架或功能直接相关的
  Markdown 页面。
- `llms-full.txt` 包含完整文档正文，适合需要一次性导入整套资料的工具；日常编码任务不必默认
  加载，以免占用过多上下文。

Skills 只维护 Agent 的目标选择、默认决策、写入与授权边界、验证和错误恢复。安装、配置、API、
框架示例与排障方法以本文档站为准，因此 Agent 遇到具体产品用法时应从 `llms.txt` 定位页面，
而不是依赖 Skill 中复制的教程。
...
3. 不要覆盖已有自动译文；
4. 列出需要人工确认的文案和建议措辞，得到确认后再写入人工校对结果；
5. 再次检查剩余缺失项并汇报。`}
/>

## 使用边界

- Agent 补译默认保留已有译文。需要修改已确认的措辞时，请明确说明“校对”或“覆盖”。
- 人工校对结果优先于自动译文。Agent 应先展示建议措辞，得到确认后再写入。
...
- 只处理某类原文或已有译文时，Agent 可以先在完整清单中定向筛选，再对结果翻页处理。
...
- 精确文案身份已经变化时，Agent 可以参考工具返回的少量候选重新定位，但不会自动把近似文案
  当成写入目标。
- Agent 工作期间不要并行手动修改同一份译文文件；完成后再进行人工校对。
- 普通补译和校对不会自动清理历史消息。只有你明确要求审查孤立消息时，Agent 才会在完整
  Build 后列出当前源码不再引用的 Translation Memory；删除前会展示结果并再次请求确认。
- 清理孤立的自动译文不会联动删除人工校对结果。需要删除孤立人工值时，应单独提出并审查。
- 生成文件和提交规则见[生成文件与 Git](/guide/basic/directory)。

MCP 提供读取、补译和人工校对所需的工具。具体工具字段属于 Agent 接入契约，通常不需要由
应用开发者手动调用。
