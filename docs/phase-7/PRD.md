...
- `translations.json` 使用 schema v1：`version`、`revision`、`messages`；每条消息显式保存
  `source`、`sourceLang`、可选 `comment` 与 `translations`。
- `overrides.json` 使用 schema v1：按 source 保存 `default`，按 comment-specific ID 保存 `byId`。
...
普通 `t(source)` 继续使用 source 生成 ID。补充翻译语境或区分同一原文语义时传静态
`{ comment }`；comment 规范化后参与 ID。
