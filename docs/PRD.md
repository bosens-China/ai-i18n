...
- 校对页面面向笔记本和桌面浏览器，手机布局不作为产品验收目标。未保存输入在当前页面会话内按
  消息、目标语言和作用范围隔离，切换筛选、搜索、语言或范围不会丢失；关闭或刷新页面不承诺保留。
- 人工校对区分“确认现有译文”“保存修改”和“已保存”，并提供保存后继续下一条的鼠标动作与
  `Command/Ctrl + Enter` 快捷键。确认未改动的自动译文或原文回退同样会写入人工 override。
