...
- `diagnostics.timing` 是默认关闭的 Vite Dev 慢阶段诊断。`true` 使用 50 ms 阈值，也可配置非负有限的
...
- Build 默认输出本轮收集的去重文案数、含文案文件数和各目标语言的有效已译/缺译数，
  ...
  均有有效译文才算已译；源语言不计入目标统计。汇总本身不触发翻译。
- 顶层 `failOnMissingTranslations` 默认关闭。开启时，Build 在 Provider 工作完成且本轮活动模块
  校准后检查全部目标语言；任一语言仍缺译，就在应用产物写出前抛错并列出各语言缺译数量。
  无 Provider 时同样生效，空字符串与有效人工覆盖算已译；与 `provider.strict` 和构建汇总开关独立。
  Build Watch 每轮重新检查，Dev、入口扫描与 Runtime 源文案回退不受影响。
...
- 性能报告累计统计覆盖全部完成样本，详细记录、进行中记录与每阶段分位数窗口均有容量上限。
  报告采用相对 root 且位于 i18n 协议目录外的独立目录，不保存源码、译文、错误正文、凭据或机器绝对
  路径；写入合并、失败隔离，报告自身文件事件不触发再次采集。旧会话报告由使用者清理。
- 插件阶段耗时不等于关闭插件后的节省时间，也不能相加作为总启动时间。仓库通过三套演示的隔离
  副本、多轮独立进程对比未注册插件、已注册但关闭采集、已注册并开启采集；保留相同配置依赖导入，
  排除 Review、Provider 网络、依赖预构建和浏览器执行，不将服务端转换基线宣称为完整页面性能。
  `examples:perf` 自动构建并启动三套普通演示观测，`perf:examples` 自动构建后进行隔离对照；对照按分析
  缓存命中拆分转换阶段，不把清理时等待后台落盘的时间列为性能指标。
  用户配置与解释见 [性能诊断](../apps/docs/docs/guide/advanced/performance.md)，维护者测量入口见
  [演示说明](../examples/README.md)。
- Vue 未开启自动导入时跳过自动导入专用编译与 AST 分析，保留消息、模板和位置映射处理。
  SFC 编译依赖可能涉及外部类型，不引入仅按当前源码内容复用的整段缓存；依赖解析仍优先保证首次结果与 HMR 正确性。
...
- 校对 UI 不依赖在线或未随包提供的 Web Font；外层入口壳与内层工作台统一使用操作系统无衬线字体，
  ...
  路径、模板占位符、语言代码、计数和快捷键；界面字重使用稳定的 600/700 档位，紧凑路径辅助文字不小于 10 px。
...
- 校对插件面向笔记本和桌面浏览器，采用左侧虚拟化消息列表和右侧编辑工作台；工作台采用紧凑
  ...
  外层标题栏与工作台主选项卡使用连续实体背景，带 Header 的校对详情再通过背景和边界保持清晰层级。
...
- 未保存输入在当前页面会话内按消息、目标语言和作用范围隔离，切换筛选、搜索、语言或范围不会
  丢失；关闭或刷新页面不承诺保留。作用范围固定为“当前位置”“当前文件”和“所有文件”三个选项卡；
  当前文件取自当前选中的出现位置。源码移动后旧位置规则不自动猜测或迁移，而作为 orphan 等待审查。
- 无当前人工覆盖时，人工输入框默认留空；自动译文只作为可显式填入的参考。仅非空且模板占位符
...
- 校对页面不引入账号或 token。写接口只接受同源、`application/json` 请求并复用 overrides 的消息、
  locale、文件归属和模板占位符校验；跨机器协作与远程暴露不属于该本地能力。
...
- 自动译文与人工覆盖写入都必须在修改存储前校验模板占位符。缺失、多出、改变编号或重复次数
  不一致时整批失败，并返回期望、实际、缺失与多余的模板占位符，供 Agent 修正后重试。
