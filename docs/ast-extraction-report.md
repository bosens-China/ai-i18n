...
> `@ai-i18n/eslint-plugin`、`@ai-i18n/react/vite`、`@ai-i18n/vue/vite`
...
| `.jsx`、`.tsx`            | 已验证         | 默认进入框架中立分析链；Hook 是否生效取决于注册的框架 extractor。                |
...
| `const { t } = useI18n(); t('保存')`             | 已验证 | 需要注册对应 Vue 或 React extractor。                    |
...
Hook import 必须来自注册 extractor 声明的精确模块：
...
React adapter 不创建第二套 AST。JSX/TSX 已默认进入共享分析链，adapter 只声明
`@ai-i18n/react/useI18n().t` 的 Hook 语义。

注册 `react()` 后，Hook 语义会作用于完整的 JS/TS 模块图，而不只作用于 React 组件文件。
...
| Vue JSX/TSX Hook                        | 已验证   | 默认 JSX 分析入口按 `@ai-i18n/vue` import binding 识别。      |

普通 `<script>` 内部的 `t()` 仍可提取，但模板需要绑定 Hook 时应使用
`<script setup lang="ts">`。外部脚本也只提取脚本中实际出现的调用，不推导 Options API
返回对象和模板之间的关系。

## 3.1 Vue JSX 与 React JSX 共存

两者可以在同一项目中出现。ai-i18n 将 JSX/TSX 作为框架中立语法解析，并根据 Hook 的
精确 import binding 自动区分 Vue 与 React，不要求文件名携带框架后缀。宿主构建不能从
通用 JSX 标签可靠判断 Runtime，因此默认由 React 处理未命中文件；Vue-only 项目关闭 React
插件，混合项目用 Vue JSX 插件的任意 `include` glob 声明 Vue 范围。同一个文件不能同时
执行两种 JSX transform，也不支持混写两个 Runtime。
...
因此不会与 `<script setup>` 中的 `translate` 淆淆。
...
- React adapter：`packages/react/src/vite.ts`
- ESLint 适配：`packages/eslint/src/analyze.ts`、`packages/eslint/src/vue-sfc.ts`
- Yuku 准入测试：`packages/vite/test/yuku-spike.test.ts`
- React adapter 测试：`packages/react/test/vite.test.ts`
- Vue scope 测试：`packages/vue/test/vite.test.ts`
- React/Vue 真实 Vite Build：`packages/react/test/integration.test.ts`、
  `packages/vue/test/integration.test.ts`
