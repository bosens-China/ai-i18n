...
> [`docs/mcp/PRD.md`](../mcp/PRD.md)；未发布的阶段性协议不要求兼容或迁移。
>
> 本阶段只实现 Build Watch、Locale Lazy 和 Cache 容量控制，不包含性能优化项目。

## 0. 实施优先级与 P0

- [x] 锁定当前默认行为、Runtime 和文件职责为回归基线。
...
- [x] 明确受保护 Translation Memory 自身超限时保留并 warning。
...
- [x] 当前默认模式、Runtime 行为和文件职责必须通过回归。
...
- [x] 静态依赖变化只刷新受影响的依赖方。
- [x] Translation Memory 或 overrides 变化不重新 parse source。
- [x] locale 作为派生产物，不反向写入翻译。
...
- [x] 内容未变化时不重写 translations、overrides、extracted 或 locale 文件。
...
- [x] 覆盖直接 source 变化。
- [x] 覆盖跨文件静态依赖变化。
- [x] 覆盖 Translation Memory 与 overrides 变化。
- [x] 覆盖 locale 派生内容刷新。
- [x] 覆盖 source 删除和重命名。
...
- [x] 保持当前 translations、overrides、extracted 和 locale 文件职责。
...
- [x] Translation Memory 或 overrides 变化只更新对应 locale。
...
- [x] 根据 extracted 结构和 ProjectState 计算活动 message IDs。
