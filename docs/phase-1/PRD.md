...
6. 默认启用 JavaScript/TypeScript/JSX/TSX 提取；Vue、React Hook 与 HTML 通过 `extractors` 注入。
...
15. 不自研 Rust 原生内核。
16. 八个公开包独立版本，使用 tsdown/Rolldown 生成 ESM 与类型声明，并以 publint、ATTW
...
- `recommended` 复用一条规则覆盖 Vanilla、React 和普通 TypeScript 中的 Vue Hook。
- `vue` 是可选 flat config，只为 `.vue` SFC 启用同一规则；宿主用 `vue-eslint-parser`
  接入文件，语义分析与 Vite 共用 compiler-sfc 结果和 source map。
...
- 默认 JS/TS/JSX/TSX 分析入口始终启用，JSX 语法本身不决定框架。
- React Hook binding 作用于完整默认模块图，并复用同一次 Yuku 分析。
- Vue Hook binding 同时作用于完整默认模块图；SFC 使用 compiler-sfc 编译后的真实
  script/setup/template 作用域和 source map，再进入统一 Yuku 分析链。
- Hook 返回值支持解构 alias 与 `i18n.t()` 成员调用；外部 Vue `<script src>` 作为独立
  JS/TS 模块进入默认分析链。
- 多个 extractor 可以同时启用并共享 ProjectState、cache、Provider 和 Runtime。
- Vue JSX 与 React JSX 可以同项目共存，ai-i18n 按 import binding 自动识别且不约束命名；
  宿主默认以 React 为 fallback，Vue-only 时关闭 React，混合时用任意 glob 声明 Vue 范围。
- 同一个文件不得混用或重复执行两种 JSX Runtime transform。
