...
- 使用标准 `fetch` 对接 OpenAI-compatible API。
- API key 只从用户配置或环境变量读取，不写入任何产物。
- Core/Vite 包不依赖 OpenAI SDK。
...
- 达到 batch size 时立即发送，不继续等待 debounce。
...
- Provider debounce 默认 100ms，batch size 默认 50，可配置。
