...
> 状态：历史实施记录（非规范；已完成当时外部验收）
>
> 本文是历史实施记录，不是当前实现规范。现行持久化、诊断与 MCP 契约分别见
...
4. 当时的 Translation Memory 保存了可由 extracted 与 ProjectState 推导的 file records。
...
- Translation Memory 不保存可由 extracted 与 ProjectState 推导的 file records。当前
  `translations.json` 的字段职责以 Phase 7 为准。
...
- Translation Memory 容量与 orphan 清理仍只删除非活跃消息。活动集合由现有 extracted 与当前
  ProjectState 计算。
...
- `translations.json` 不含可由 extracted 与 ProjectState 推导的 file records。
